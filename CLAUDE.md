...
The v0.x line covers: stdio transport only; assets + attributes + asset-model; realms (read in v0.1.0, full CRUD in v0.2.0); users (Keycloak-backed: CRUD + roles + password-reset email flow, v0.2.0; sessions, direct-credential mutation, and self-service profile/locale writes excluded. Sessions/credentials are destructive footguns with LLM-context credential leakage; self-service writes (`update_current_user`, `update_current_user_locale`, `request_current_password_reset`) are useless under service-account auth: `update_current_user` 405s because the body's `isServiceAccount` defaults to false while the stored record is a service account, `update_current_user_locale` is a no-op with no UI session, and `request_current_password_reset` emails the service account itself, which has no inbox); system status (health + info, v0.2.0); syslog (events + config, v0.2.0).
